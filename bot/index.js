...
🚀 Comece agora mesmo registrando um gasto!
  ...
  async handleNewMessages(messages) {
    for (const message of messages) {
      ...
      if (this.userManager.isNewUser(sender)) {
        await this.sendMessage(sender, this.userManager.getWelcomeMessage(sender));
      }
      ...
      try {
        ...
        await this.sendMessage(sender, '❌ Ocorreu um erro ao processar sua mensagem. Tente novamente.');
      }
    }
  }
  ...
  async handleExpenseMessage(sender, text, senderName) {
    ...
    if (parsed) {
      const expenseId = await this.db.addExpense(sender, parsed.value, parsed.description, parsed.category);
...
_Use /relatorio para ver o resumo completo_
      ...
      // Log para controle
      console.log(`💰 Gasto registrado: ${senderName} - R$ ${parsed.value} - ${parsed.description}`);
...
_Use /ajuda para ver todos os comandos_
  ...
  async handleCommand(sender, command, senderName) {
    ...
    switch (cmd) {
      ...
      default:
        await this.sendMessage(sender, '❌ Comando não reconhecido. Use /ajuda para ver os comandos disponíveis.');
    }
  }
  ...
  async handleDeleteCommand(sender, command) {
    ...
    if (args.length === 0) {
      ...
      if (recentExpenses.length === 0) {
        await this.sendMessage(sender, '📝 Nenhum gasto encontrado para deletar.');
        return;
      }
      ...
      recentExpenses.forEach(expense => {
        ...
        message += `${icon} *#${expense.id}* - R$ ${expense.value.toFixed(2)}\n`;
    ...
    try {
      ...
      if (!expense) {
        await this.sendMessage(sender, `❌ Gasto #${expenseId} não encontrado.\n\nUse /deletar para ver seus gastos recentes.`);
        return;
      }
...
_Use /relatorio para ver o resumo atualizado_
        ...
        await this.sendMessage(sender, '❌ Erro ao deletar o gasto. Tente novamente.');
      }
      
    } catch (error) {
      console.error('❌ Erro ao deletar gasto:', error);
      await this.sendMessage(sender, '❌ Erro interno ao deletar gasto. Tente novamente.');
    }
  }
...
_Use /recentes para ver os IDs dos seus gastos_
    ...
    try {
      ...
      if (!parsed) {
        await this.sendMessage(sender, `❌ Formato inválido.\n\n*Use:* /editar ${expenseId} [valor] [descrição] [categoria]`);
        return;
      }
      
      // Atualizar o gasto
      const updated = await this.db.updateExpense(sender, expenseId, parsed.value, parsed.description, parsed.category);
...
_Use /relatorio para ver o resumo atualizado_
        ...
        await this.sendMessage(sender, '❌ Erro ao editar o gasto. Tente novamente.');
      }
      
    } catch (error) {
      console.error('❌ Erro ao editar gasto:', error);
      await this.sendMessage(sender, '❌ Erro interno ao editar gasto. Tente novamente.');
    }
  }
...
_Busca por descrição, valor ou categoria_
    ...
    try {
      ...
      if (results.length === 0) {
        await this.sendMessage(sender, `🔍 Nenhum resultado encontrado para: "${searchTerm}"\n\n_Tente outros termos de busca_`);
        return;
      }
      ...
      await this.sendMessage(sender, '❌ Erro ao realizar busca. Tente novamente.');
    }
  }
  ...
  async sendRecentExpenses(sender) {
    try {
      ...
      if (recentExpenses.length === 0) {
        await this.sendMessage(sender, '📝 Nenhum gasto registrado ainda.\n\n_Comece registrando um gasto!_');
        return;
      }
      ...
      recentExpenses.forEach((expense, index) => {
        const date = new Date(expense.date).toLocaleDateString('pt-BR');
        const icon = this.getCategoryIcon(expense.category);
        message += `${index + 1}. ${icon} *#${expense.id}* - R$ ${expense.value.toFixed(2)}\n`;
        message += `   📝 ${expense.description}\n`;
        message += `   📅 ${date} • 🏷️ ${expense.category}\n\n`;
      });
...
*Exemplo:* /limpar hoje CONFIRMAR
    ...
    if (confirmation !== 'CONFIRMAR') {
      await this.sendMessage(sender, '❌ Para confirmar a limpeza, adicione CONFIRMAR ao final do comando.\n\n*Exemplo:* /limpar hoje CONFIRMAR');
      return;
    }
...
_Use /relatorio para ver o resumo atualizado_
      ...
      await this.sendMessage(sender, '❌ Erro ao limpar dados. Tente novamente.');
    }
  }

  async sendReport(sender, period) {
    try {
      const report = await this.reportGenerator.generateTextReport(sender, period);
      await this.sendMessage(sender, report);
      
      // Opcionalmente, enviar gráfico
      const chartPath = await this.reportGenerator.generateChart(sender, period);
      if (chartPath) {
      ...
      await this.sendMessage(sender, '❌ Erro ao gerar relatório. Tente novamente.');
    }
  }
  ...
  async sendCategoriesReport(sender) {
    try {
      ...
      await this.sendMessage(sender, '❌ Erro ao gerar relatório de categorias.');
    }
  }
...
👤 *Seu perfil:*
   ...
   • Membro desde: ${userInfo ? new Date(userInfo.firstSeen).toLocaleDateString('pt-BR') : 'Hoje'}
...
🔄 Última atualização: ${new Date().toLocaleString('pt-BR')}
  ...
  async handleAdminCommand(sender, args) {
    ...
    if (!adminNumbers.includes(sender)) {
      await this.sendMessage(sender, '❌ Comando disponível apenas para administradores.');
      return;
    }
    ...
    if (args.length === 0 || args[0] === 'users') {
      ...
      await this.sendMessage(sender, `🧹 Limpeza concluída: ${cleanedCount} usuários inativos removidos.`);
    } else {
      await this.sendMessage(sender, `
🔧 *COMANDOS ADMIN*
━━━━━━━━━━━━━━━━━━━━━━
